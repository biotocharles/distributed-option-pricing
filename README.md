...
The system is built on a Master-Worker Architecture designed to handle the O(MNK^2) complexity of the Longstaff-Schwartz method.

  # Phase 1: Numerical Core

   Path Simulation: Geometric Brownian Motion (GBM) path generation.
   
   Regression Engine: High-performance polynomial regression using Eigen's Cholesky Decomposition to solve the normal equations (
